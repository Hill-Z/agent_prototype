...
function GlobalSidebar({ notify, openSkills }: { notify: (message: string) => void; openSkills: () => void }) {
  const [active, setActive] = useState(1);
  return <aside className="global-sidebar" aria-label="全局导航">{navItems.map((Icon, index) => <button key={index} className={`sidebar-icon ${index === active ? 'active' : ''}`} title={index === 12 ? '技能' : `导航 ${index + 1}`} onClick={() => { if (index === 12) openSkills(); else { setActive(index); notify(`已切换到导航 ${index + 1}`); } }}><Icon size={17} strokeWidth={1.8} /></button>)}<span className="sidebar-spacer" /><button className="sidebar-icon" title="收起导航"><PanelLeftClose size={17} /></button></aside>;
}
