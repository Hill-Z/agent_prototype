...
function AgentHeader({ tab, setTab, config, setConfig, notify, openPublish, restore, onBack }: { tab: WorkspaceTab; setTab: (tab: WorkspaceTab) => void; config: AgentConfig; setConfig: React.Dispatch<React.SetStateAction<AgentConfig>>; notify: (message: string) => void; openPublish: () => void; restore: () => void; onBack: () => void }) {
  ...
  return <header className="agent-header"><div className="agent-identity"><button className="icon-button" aria-label="返回我的应用" onClick={onBack}><ArrowLeft size={19} /></button><div className="agent-chip"><span className="agent-avatar" /><div><strong>new高级智能体</strong><small>高级智能体</small></div><Settings size={15} /></div></div><nav className="agent-tabs">{tabLabels.map(([value, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}</button>)}</nav><div className="agent-actions">{tab === 'orchestration' ? <><button className="model-selector" onClick={() => setModelOpen(value => !value)}><Bot size={16} /><span>{config.model.id}</span><em>CHAT</em><SlidersHorizontal size={14} /></button><button className="icon-button" onClick={restore} aria-label="恢复已保存配置"><History size={17} /></button><button className="secondary-button" onClick={() => { saveAgentSnapshot(config); notify('配置已保存'); }}><Save size={15} />保存</button><button className="primary-button" onClick={openPublish}>发布<ChevronDown size={14} /></button>{modelOpen ? <ModelPanel config={config} setConfig={setConfig} close={() => setModelOpen(false)} /> : null}</> : null}</div></header>;
}
