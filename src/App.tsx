...
import { createId, type AgentConfig, type ModelParameters, type WorkspaceTab } from './features/agent/agent.types';
...
function GlobalSidebar({ activeArea, notify, openArea }: { activeArea: 'agent' | 'memory' | 'channels'; notify: (message: string) => void; openArea: (area: 'agent' | 'memory' | 'channels' | 'skills' | 'tools' | 'cards' | 'apps') => void }) {
...
const modelOptions = ['Doubao-Seed-2.0-pro', 'gpt-4o-mini', 'DeepSeek-V3'];
const modelMeta: Record<string, { description: string; tags: string[] }> = {
  'Doubao-Seed-2.0-pro': { description: '综合能力强，适合客户对话与工具调用', tags: ['通用', '工具调用', '视觉'] },
  'DeepSeek-V3': { description: '复杂任务与深度推理', tags: ['深度推理', '工具调用'] },
  'gpt-4o-mini': { description: '响应快，适合低延迟场景', tags: ['快速', '低成本'] }
};
const defaultModelParameters = (): ModelParameters => ({ preset: 'balanced', temperatureEnabled: false, temperature: 0, topPEnabled: false, topP: 0, frequencyEnabled: false, frequencyPenalty: 0, presenceEnabled: false, presencePenalty: -2, maxTokensEnabled: false, maxTokens: 1, thinking: false });
...
function ModelPanel({ config, setConfig, close }: { config: AgentConfig; setConfig: React.Dispatch<React.SetStateAction<AgentConfig>>; close: () => void }) {
  ...
  const updatePool = (pool: AgentConfig['modelRouting']['pool']) => setConfig(current => ({ ...current, model: { ...current.model, id: pool[0]?.modelId ?? current.model.id }, modelRouting: { ...current.modelRouting, pool } }));
  const updateParameters = (itemId: string, patch: Partial<ModelParameters>) => updatePool(config.modelRouting.pool.map(item => item.id === itemId ? { ...item, parameters: { ...item.parameters, ...patch } } : item));
  const moveModel = (index: number, offset: number) => { const next = [...config.modelRouting.pool]; const target = index + offset; if (target < 0 || target >= next.length) return; [next[index], next[target]] = [next[target], next[index]]; updatePool(next); };
  const addModel = () => { const nextModel = modelOptions.find(model => !config.modelRouting.pool.some(item => item.modelId === model)); if (!nextModel || config.modelRouting.pool.length >= 3) return; updatePool([...config.modelRouting.pool, { id: createId('model'), modelId: nextModel, parameters: defaultModelParameters() }]); };
  const metrics: Array<[string, keyof ModelParameters, keyof ModelParameters, number, number, number]> = [['温度', 'temperatureEnabled', 'temperature', 0, 2, .1], ['Top P', 'topPEnabled', 'topP', 0, 1, .1], ['频率惩罚', 'frequencyEnabled', 'frequencyPenalty', -2, 2, .1], ['存在惩罚', 'presenceEnabled', 'presencePenalty', -2, 2, .1], ['最大标记', 'maxTokensEnabled', 'maxTokens', 1, 8192, 1]];
  return <div className="model-panel model-routing-panel" role="dialog" aria-label="模型配置"><header><div><strong>模型配置</strong><small>配置可用模型，系统按顺序保障运行</small></div><button className="icon-button" onClick={close}>×</button></header><section className="model-pool"><div className="model-pool-heading"><div><strong>模型池</strong><small>最多添加 3 个模型，调整顺序决定优先级</small></div><button className="small-button" disabled={config.modelRouting.pool.length >= 3} onClick={addModel}><Plus size={13} />添加模型</button></div>{config.modelRouting.pool.map((item, index) => { const meta = modelMeta[item.modelId]; return <div className="model-pool-item" key={item.id}><span className="model-priority">{index === 0 ? '主模型' : `备用 ${index}`}</span><div className="model-pool-content"><select value={item.modelId} onChange={event => updatePool(config.modelRouting.pool.map((current, currentIndex) => currentIndex === index ? { ...current, modelId: event.target.value } : current))}>{modelOptions.filter(model => model === item.modelId || !config.modelRouting.pool.some(existing => existing.modelId === model)).map(model => <option key={model}>{model}</option>)}</select><small>{meta?.description}</small><div className="model-capabilities">{meta?.tags.map(tag => <span key={tag}>{tag}</span>)}</div><button className="model-parameter-trigger" onClick={() => setEditingModelId(current => current === item.id ? null : item.id)}>参数设置<ChevronDown size={13} /></button>{editingModelId === item.id ? <div className="model-item-tuning"><div className="preset-control">{[['precise', '精准模式'], ['balanced', '平衡模式'], ['creative', '创意模式']].map(([value, label]) => <button className={item.parameters.preset === value ? 'active' : ''} key={value} onClick={() => updateParameters(item.id, { preset: value })}>{label}</button>)}</div>{metrics.map(([label, enabledKey, valueKey, min, max, step]) => <div className="model-metric" key={label}><span>{label}</span><Switch checked={Boolean(item.parameters[enabledKey])} onChange={value => updateParameters(item.id, { [enabledKey]: value })} label={`启用${label}`} /><input type="range" min={min} max={max} step={step} value={Number(item.parameters[valueKey])} onChange={event => updateParameters(item.id, { [valueKey]: Number(event.target.value) })} /><input type="number" min={min} max={max} step={step} value={Number(item.parameters[valueKey])} onChange={event => updateParameters(item.id, { [valueKey]: Number(event.target.value) })} /></div>)}<small>未开启的参数使用该模型的推荐值。</small></div> : null}</div><div className="model-pool-actions"><button className="icon-button" disabled={index === 0} onClick={() => moveModel(index, -1)} aria-label="上移"><ArrowUp size={14} /></button><button className="icon-button" disabled={index === config.modelRouting.pool.length - 1} onClick={() => moveModel(index, 1)} aria-label="下移"><ArrowDown size={14} /></button><button className="icon-button danger" disabled={config.modelRouting.pool.length === 1} onClick={() => updatePool(config.modelRouting.pool.filter(current => current.id !== item.id))} aria-label="删除"><Trash2 size={14} /></button></div></div>; })}</section><div className="model-policy-note"><Bot size={17} /><div><strong>系统自动处理模型选择</strong><p>正常使用主模型；主模型不可用时按顺序切换。单 Agent 直接使用本模型池，多 Agent 中子 Agent 默认继承，也可在子 Agent 内覆盖。</p></div></div><label className="routing-retry">同模型重试次数<input type="number" min={0} max={3} value={config.modelRouting.retryCount} onChange={event => setConfig(current => ({ ...current, modelRouting: { ...current.modelRouting, retryCount: Number(event.target.value) } }))} /><small>重试后仍不可用，再切换备用模型</small></label></div>;
}

function AgentHeader({ tab, setTab, config, setConfig, notify, openPublish, restore, onBack }: { tab: WorkspaceTab; setTab: (tab: WorkspaceTab) => void; config: AgentConfig; setConfig: React.Dispatch<React.SetStateAction<AgentConfig>>; notify: (message: string) => void; openPublish: () => void; restore: () => void; onBack: () => void }) {
  const [modelOpen, setModelOpen] = useState(() => new URLSearchParams(window.location.search).get('panel') === 'model');
  const activeModel = config.modelRouting.pool[0]?.modelId ?? config.model.id;
  return <header className="agent-header"><div className="agent-identity"><button className="icon-button" aria-label="返回我的应用" onClick={onBack}><ArrowLeft size={19} /></button><div className="agent-chip"><span className="agent-avatar" /><div><strong>new高级智能体</strong><small>高级智能体</small></div><Settings size={15} /></div></div><nav className="agent-tabs">{tabLabels.map(([value, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}</button>)}</nav><div className="agent-actions">{tab === 'orchestration' ? <><button className="model-selector" onClick={() => setModelOpen(value => !value)}><Bot size={16} /><span>{activeModel}</span><em>CHAT</em><SlidersHorizontal size={14} /></button><button className="icon-button" onClick={restore} aria-label="恢复已保存配置"><History size={17} /></button><button className="secondary-button" onClick={() => { saveAgentSnapshot(config); notify('配置已保存'); }}><Save size={15} />保存</button><button className="primary-button" onClick={openPublish}>发布<ChevronDown size={14} /></button>{modelOpen ? <ModelPanel config={config} setConfig={setConfig} close={() => setModelOpen(false)} /> : null}</> : null}</div></header>;
}
