...
const navItems = [
  { icon: Layers3, label: '应用' }, { icon: CircleGauge, label: '高级智能体' }, { icon: Database, label: '知识库' }, { icon: BrainCircuit, label: '记忆库' }, { icon: Archive, label: '专业词库' }, { icon: ListChecks, label: '问答对' }, { icon: Play, label: '工作流' }, { icon: FileText, label: '文档' }, { icon: TableProperties, label: '数据表' }, { icon: Blocks, label: 'MCP' }, { icon: UserRoundCheck, label: '人工审核' }, { icon: Tags, label: '标签' }, { icon: Wrench, label: '工具', area: 'tools' as const }, { icon: WandSparkles, label: '技能', area: 'skills' as const }, { icon: GalleryVerticalEnd, label: '资源' }, { icon: Clock3, label: '历史' }, { icon: SlidersHorizontal, label: '设置' }
];
...
function GlobalSidebar({ notify, openSkills, openTools }: { notify: (message: string) => void; openSkills: () => void; openTools: () => void }) {
  const [active, setActive] = useState(1);
  return <aside className="global-sidebar" aria-label="全局导航">{navItems.map(({ icon: Icon, label, area }, index) => <button key={label} className={`sidebar-icon ${index === active ? 'active' : ''}`} aria-label={label} title={label} onClick={() => { if (area === 'skills') openSkills(); else if (area === 'tools') openTools(); else { setActive(index); notify(`已切换到${label}`); } }}><Icon size={17} strokeWidth={1.8} /></button>)}<span className="sidebar-spacer" /><button className="sidebar-icon" title="收起导航"><PanelLeftClose size={17} /></button></aside>;
}
...
function ConfigurationPane({ config, setConfig, dialog, guardrail, dispatch }: { config: AgentConfig; setConfig: React.Dispatch<React.SetStateAction<AgentConfig>>; dialog: (kind: DialogKind) => void; guardrail: ReturnType<typeof loadGuardrailConfig>; dispatch: React.Dispatch<Parameters<typeof guardrailReducer>[1]> }) {
  ...
  return <div className="configuration-pane">
    ...
    <ConfigSection title="长期记忆" icon={MemoryStick}><div className="setting-line"><strong>启用长期记忆</strong><Switch checked={config.longMemory.enabled} onChange={enabled => update('longMemory', { ...config.longMemory, enabled })} label="启用长期记忆" /></div>{config.longMemory.enabled ? <div className="form-grid"><label>Agent 记忆库<select value={config.longMemory.agentStore} onChange={e => update('longMemory', { ...config.longMemory, agentStore: e.target.value })}><option value="">请选择</option><option>客服经验记忆库</option></select></label><label>用户记忆库<select value={config.longMemory.userStore} onChange={e => update('longMemory', { ...config.longMemory, userStore: e.target.value })}><option value="">请选择</option><option>客户画像记忆库</option></select></label><label>召回条数<input type="number" value={config.longMemory.recallCount} onChange={e => update('longMemory', { ...config.longMemory, recallCount: Number(e.target.value) })} /></label></div> : null}</ConfigSection>
...
export default function App() {
  ...
  const [area, setArea] = useState<'agent' | 'skills' | 'tools'>(() => { const view = new URLSearchParams(window.location.search).get('view'); return view === 'skills' || view === 'tools' ? view : 'agent'; });
  ...
  return <div className="app-shell"><TopHeader />{area === 'skills' ? <SkillManagementPage onBack={() => setArea('agent')} onOpenTools={() => setArea('tools')} /> : area === 'tools' ? <ToolManagementPage onBack={() => setArea('agent')} onOpenSkills={() => setArea('skills')} /> : <><div className="app-body"><GlobalSidebar notify={setToast} openSkills={() => setArea('skills')} openTools={() => setArea('tools')} /><main className="workbench"><AgentHeader tab={tab} setTab={setTab} config={agentConfig} setConfig={setAgentConfig} notify={setToast} openPublish={() => setDialog('publish')} restore={restore} />{tab === 'orchestration' ? <ResizableWorkspace><ConfigurationPane config={agentConfig} setConfig={setAgentConfig} dialog={setDialog} guardrail={guardrail} dispatch={dispatch} /><RuntimePreviewPanel guardrail={guardrail} responseExperience={agentConfig.responseExperience} conversationBehavior={agentConfig.conversationBehavior} proactiveService={agentConfig.proactiveService} multimodal={agentConfig.multimodal} /></ResizableWorkspace> : tab === 'channels' ? <ChannelWorkspace notify={setToast} /> : tab === 'memory' ? <MemoryWorkspace config={agentConfig.longMemory} update={patch => setAgentConfig(current => ({ ...current, longMemory: { ...current.longMemory, ...patch } }))} notify={setToast} /> : tab === 'api' ? <ApiDocsView notify={setToast} /> : tab === 'logs' ? <LogsView /> : tab === 'monitor' ? <RealtimeMonitorView /> : tab === 'reports' ? <ReportsView /> : <ReviewView />}</main></div><ConfigDialogs kind={dialog} config={agentConfig} setConfig={setAgentConfig} close={() => setDialog(null)} notify={setToast} /></>}{toast ? <Toast>{toast}</Toast> : null}</div>;
}
