...
function GlobalSidebar({ activeArea, notify, openArea }: { activeArea: 'agent' | 'memory' | 'channels'; notify: (message: string) => void; openArea: (area: 'agent' | 'memory' | 'channels' | 'skills' | 'tools') => void }) {
...
export default function App() {
  ...
  const [area, setArea] = useState<'agent' | 'memory' | 'channels' | 'skills' | 'tools'>(() => { const view = new URLSearchParams(window.location.search).get('view'); return view === 'skills' || view === 'tools' || view === 'memory' || view === 'channels' ? view : 'agent'; });
