...
_ESCAPED_HTML_START = re.compile(
    r"^\s*&(?:amp;)*lt;(?:!doctype|html|body|div|p|table|h[1-6])\b",
    re.IGNORECASE,
)


def _decode_escaped_html_document(content: str) -> str:
    """Decode HTML entities only when the entire payload is an escaped HTML document."""
    value = _normalize_text(content)
    for _ in range(3):
        if not _ESCAPED_HTML_START.search(value):
            break
...
def _extract_prepared_email(subject: str, email_content: str):
    ...
    content = _decode_escaped_html_document(content)
