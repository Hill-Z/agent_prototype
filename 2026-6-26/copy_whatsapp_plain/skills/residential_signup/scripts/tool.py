...
For your profile, {_plan_label(recommended['label'])} is a good fit because it gives you fixed-price certainty over {recommended['contract']}.

{_note('The rates above are indicative. Final plan details and eligibility will be confirmed during online sign-up.')}"""
...
按您当前情况，{_plan_label(recommended['label'])} 会比较合适，因为它可以在 {recommended.get('contract_zh', recommended['contract'])} 内提供固定价格确定性，月度费用更容易预估。

{_note('以上为参考计划信息，最终计划详情和适用资格以在线开户注册页面确认为准。')}"""
...
Sign-up Link: Continue residential sign-up
URL: {link}
