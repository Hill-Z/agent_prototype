...
def _normalize_date(value=""):
    """Normalize an explicit current/future year-month-day value."""
...
def _resolve_move_dates(move_out_date="", new_supply_date="", effective_date=""):
    ...
    legacy_dates = [value for value in legacy_dates if value]
...
def submit_account_update(
    ...
    """Submit an account update or move-house service request as customer-facing plain text."""
    en = _is_en(language)
    c = _customer(account_no)
    if not c:
