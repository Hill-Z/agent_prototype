...
def query_crm(account_no: str = "", customer_id: str = "", phone: str = "", email: str = "") -> str:
    """查询 PacificLight CRM 客户档案。"""
    c = _find_customer(account_no=account_no, customer_id=customer_id, phone=phone, email=email)
    if not c:
        return "未找到匹配的 CRM 客户档案。"
    ...
    lines = [
        ...
        f"服务地址：{c['service_address']}",
    ]
    if c.get("assigned_sales"):
        lines.append(f"负责销售：{c['assigned_sales']}")
    return "\n".join(lines)
