...
def verify_identity(account_no: str = "", postal_code: str = "", phone: str = "", email: str = "", id_last4: str = "", language: str = "zh") -> str:
    """验证 PacificLight 客户身份。传入账号/手机号/邮箱，以及邮编或证件后四位。"""
    en = str(language or "").lower().startswith("en")
    c = _find_customer(account_no=account_no, phone=phone, email=email)
    if not c:
        return "Verification failed: no matching customer was found. Please check the account number, phone number or email address." if en else "验证失败：未找到匹配客户。请确认账户号、手机号或邮箱。"
    matched = []
    if postal_code and postal_code == c.get("postal_code"):
        matched.append("postal_code")
    if id_last4 and id_last4 == c.get("id_last4"):
        matched.append("id_last4")
    if not matched:
        return "Verification failed: the account was found, but the verification details do not match. Please check the postal code or verification information." if en else "验证失败：账户存在，但验证信息不匹配。请确认邮编或验证信息。"
