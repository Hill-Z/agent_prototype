...
def _payment_note(bill, language="zh"):
    ...
    if _is_en(language):
        if status == "paid":
            return "This bill has been paid. No further payment is required. You may keep this bill as your payment record."
        return f"Please complete the payment before **{bill['due_date']}** to avoid late payment charges."
    if status == "paid":
        return f"该账单已支付，无需再次付款。您可以保存本账单作为付款记录。"
    return f"请在 **{bill['due_date']}** 前完成付款，以避免产生 Late Payment Charge。"


def _build_markdown_bill(customer, bill, verified, language="zh"):
...
| Field | Details |
|---|---|
| Customer Name / Company | {display_name} |
...
| Bill Status | {_status_label(bill['status'], language)} |
...
| Payment Status | {_status_label(bill['status'], language)} |
...
| 账单状态 | {_status_label(bill['status'], language)} |
...
| 出账日期 | {bill['bill_date']} |
| 到期日 | {bill['due_date']} |
| 付款状态 | {_status_label(bill['status'], language)} |
| Late Payment Charge | {_money(bill['late_payment_charge'])} |
...
| 抄表方式 | {bill['reading_type']} |

### 费用明细

| 项目 | 金额 |
|---|---:|
| 电费 Energy Charge | {_money(bill['energy_charge'])} |
| 市场支持服务费 MSS Charge | {_money(bill['mss_charge'])} |
| 电表服务费 Metering Charge | {_money(bill['metering_charge'])} |
| 碳税 Carbon Tax | {_money(bill['carbon_tax'])} |
| 小计 Subtotal | {_money(subtotal)} |
| GST 9% | {_money(gst)} |
| Late Payment Charge | {_money(bill['late_payment_charge'])} |
| **应付总额 Total Amount Due** | **{_money(total)}** |
...
def query_bill(account_no: str = "", customer_id: str = "", period: str = "latest", postal_code: str = "", id_last4: str = "", language: str = "zh") -> str:
    """查询 PacificLight 电费账单，并返回可直接展示的 Markdown FINAL_ANSWER。"""
    ...
    return "FINAL_ANSWER: " + _build_markdown_bill(c, b, verified, language)
