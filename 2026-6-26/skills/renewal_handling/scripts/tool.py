...
def handle_renewal(account_no: str = "", postal_code: str = "", requested_plan: str = "", contract_duration_months: str = "24") -> str:
    """处理 PacificLight 续约请求。"""
    c = _find_customer(account_no=account_no)
    if not c:
        return "未找到客户，无法查询续约资格。"
    if postal_code and postal_code != c.get("postal_code"):
        return "验证失败：邮编不匹配，无法查询续约信息。"
    display_name = c.get("company") or c.get("name")
    return (f"续约意向已记录。\n"
            f"客户：{display_name}\n"
            f"账户号：{c['account_no']}\n"
            f"当前套餐：{c['current_plan']}\n"
            f"合同到期日：{c['contract_end_date']}\n"
            f"期望套餐：{requested_plan or c['current_plan']}\n"
            f"期望期限：{contract_duration_months} 个月\n"
            f"后续动作：由 PacificLight 相关团队确认正式续约条款。")
